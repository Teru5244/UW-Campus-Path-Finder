...
public final class RatPoly {
    ...
    /**
     * This is the helper function to copy the original values from this to a new RatPoly object.
     *
     * @return a copy of values of "this" to a new RatPoly object
     */
    private RatPoly copy_this() {
        RatPoly r = new RatPoly();
        for (RatTerm t: terms) {
            r.terms.add(t);
        }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        for (int i = 0; i < this.terms.size(); i++) {
            for (int j = 0; j < p.terms.size(); j++) {
                sortedInsert(r.terms, this.terms.get(i).mul(p.terms.get(j)));
            }
        }
        r.checkRep();
        return r;
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly r = new RatPoly();
        RatPoly u = copy_this();
        while (!u.terms.isEmpty() && u.degree() >= p.degree()) {
            RatTerm t = u.terms.get(0).div(p.terms.get(0));
            r.terms.add(t);
            u = u.sub(p.mul(new RatPoly(t)));
        }
        r.checkRep();
        return r;
    }

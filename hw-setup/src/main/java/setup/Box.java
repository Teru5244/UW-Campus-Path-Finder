...
public class Box implements Iterable<Ball> {
    ...
    private Map<Double, Ball> sortedMap;
    ...
    public Box(double maxVolume) {
        ...
        this.volumeList = new ArrayList<Double>();
        this.sortedMap = new HashMap<Double, Ball>();
    }
    ...
    public boolean add(Ball b) {
        ...
        if (!ballContainer.contains(b) && (ballContainer.getVolume() + b.getVolume() <= maxVolume)) {
            ballContainer.add(b);
            volumeList.add(b.getVolume());
            sortedMap.put(b.getVolume(), b);
            return true;
        }
        return false;
    }
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ...
        for (int i = 0; i < volumeList.size(); i++) {
            sorted.add(sortedMap.get(volumeList.get(i)));
        }
